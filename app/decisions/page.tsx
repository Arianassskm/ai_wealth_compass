...
export default function DecisionsPage() {
  ...
  const handleInstallmentSelect = (value: number, unit: 'month' | 'year') => {
    setSelectedInstallment({ value, unit })
    setSelectedPaymentMethod('installment')
    console.log(`Selected ${value} ${unit}s for amount:`, amount)
  }
  ...
  const handleAIEvaluation = async () => {
    ...
    if (!selectedPaymentMethod) {
      setShowPaymentMethodReminder(true);
      setTimeout(() => setShowPaymentMethodReminder(false), 2000);
      return;
    }
    ...
    // 生成一个新的评估 ID，这里我们使用时间戳作为示例
    const newEvaluationId = Date.now().toString();
    router.push(`/ai-evaluation/${newEvaluationId}?amount=${amount}&paymentMethod=${selectedPaymentMethod}`);
  };
  ...
  const handleLabelComplete = (label: string, type: string) => {
    ...
    // Here you can add logic to handle the completed label
  }
